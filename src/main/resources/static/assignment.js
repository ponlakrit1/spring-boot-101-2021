...
function resetButtonOnClick(){
    ...
    document.getElementById("statement-type-search").value = "";

    getStatementData();
}

function onChangeStatementTypeSearch(){
    let type = document.getElementById("statement-type-search").value;

    if(type == null || type == ""){
        getStatementData();
    } else {
        fetch(ROOT_URL+"/statement/find/type/"+type)
            .then(function(response) {
                response.json().then(function(data) {
                    statementList = data;
